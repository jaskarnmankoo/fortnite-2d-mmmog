...
app.post("/api/register/", function (req, res) {
	...
	var score = 1
	let sql =
		"INSERT INTO player(username, password, email, birthday, year, lecture, score) VALUES (?,?,?,?,?,?,?);"

	bcrypt.hash(password, saltRounds, function (err, hash) {
		if (hash) {
			db.run(
				sql,
				[username, hash, email, birthday, year, lecture, score],
				function (err) {
					var result = {}
					if (err) {
						res.status(409)
...
app.put("/api/update/user/:username", function (req, res) {
	...
	var lecture = req.body.lecture
	let sql =
		"UPDATE player SET password=?, email=?, birthday=?, year=?, lecture=? WHERE username=?"

	bcrypt.hash(password, saltRounds, function (err, hash) {
		if (hash) {
			db.run(
				sql,
				[hash, email, birthday, year, lecture, username],
				function (err) {
					var result = {}
					if (err) {
						res.status(404)
