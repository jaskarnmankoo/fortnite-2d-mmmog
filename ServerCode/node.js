...
class GameModel {
	...
	createWeapon_Gun(style) {
		...
		weapon.addBulletSpawn(
			new BulletSpawn(this, new Pair(0, 0), 1, 1, 0, new NullShape(), [
				bulletShape,
			])
		)
		return weapon
	...
	createWeapon_DualPistol(style) {
		...
		var bulletShape = this.createBulletShape(4, 6, new ColourCStyle(0, 0, 0, 1))
		weapon.addBulletSpawn(
			new BulletSpawn(this, new Pair(6, -3), 1, 1, -ang, new NullShape(), [
				bulletShape,
			])
		)
		weapon.addBulletSpawn(
	...
	createWeapon_Shotgun(style) {
		...
		var bulletShape = this.createBulletShape(4, 6, new ColourCStyle(0, 0, 0, 1))
		weapon.addBulletSpawn(
			new BulletSpawn(this, new Pair(0, -16), 1, 6, 0, new NullShape(), [
				bulletShape,
			])
		)
		return weapon
	...
	createWeapon_DualSMG(style) {
		...
		var bulletShape = this.createBulletShape(4, 6, new ColourCStyle(0, 0, 0, 1))
		weapon.addBulletSpawn(
			new BulletSpawn(this, new Pair(2, -4), 1, 1, -ang, new NullShape(), [
				bulletShape,
			])
		)
		weapon.addBulletSpawn(
	...
	createWeapon_Sniper(style) {
		...
		weapon.addBulletSpawn(
			new BulletSpawn(this, new Pair(0, -36), 1, 1, 0, new NullShape(), [
				bulletShape,
			])
		)
		return weapon
	...
	setStage(stageName) {
		if (stageName == "Basic") {
			...
			this.playerSpawnLocations = [
				...
				// center right
				new Pair(
					Math.floor(this.width / 2) + Math.floor(offsetL / 2),
					Math.floor(this.height / 2)
				),
				...
				new Pair(
					Math.floor(this.width / 2),
					Math.floor(this.height / 2) + Math.floor(offsetH / 2)
				),
			...
			var t1 = this.createTeleporter(
				new Pair(offsetL + 100, offsetH + 100),
				null
			) // top left corner
			...
			this.itemSpawnLocations = [
				...
				new Pair(
					this.mapCenter.x + offsetL + offsetL,
					this.mapCenter.y + offsetH + offsetH
				),
				new Pair(
					this.mapCenter.x - offsetL - offsetL,
					this.mapCenter.y - offsetH - offsetH
...
class Actor {
	...
	getTruePosition() {
		...
		var rootPosition =
			parentActor == null ? this.position : parentActor.getTruePosition()
	...
	getTrueRotation() {
		...
		return parentActor == null
			? this.rotationAngle
			: this.rotationAngle + parentActor.getTrueRotation()
	}
...
class MovingActor extends Actor {
	...
	step() {
		...
		var scaleFactor =
			terrain != null && this.affectedByTerrain ? terrain.speedMod : 1
		x *= scaleFactor
		y *= scaleFactor
		var canMove = true

		var startingPosition = this.getTruePosition()
		var collidingActors = _rooms[this.stageID].model.getCollidingActors(
			startingPosition.plus(new Pair(x, y)),
			this
		)
		for (var j = 0; j < collidingActors.length; j++) {
...
class ActorSimple {
	constructor(actor) {
		...
		this.buildingBlock =
			actor.buildingBlock == null ? null : new ActorSimple(actor.buildingBlock)
		...
		this.crosshairs =
			actor.crosshairs == null ? null : new ActorSimple(actor.crosshairs)
