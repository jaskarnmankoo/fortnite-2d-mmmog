class GameView {
	...
	drawHPBar(context, actor, colour) {
		...
		context.fillRect(
			actor.x + this.positionOffset.x - 35,
			actor.y + this.positionOffset.y + actor.height / 2 + 8,
			70,
			6
		)
		...
		if (actor.actorType == "Player") {
			...
			context.fillText(
				actor.playerID,
				actor.x + this.positionOffset.x,
				actor.y + this.positionOffset.y - actor.height / 2 - 12
			)
		}
	}
	...
	drawMinimap(context) {
		...
		if (this.playerActor) {
			...
			shape.draw(
				...
				this.playerActor.rotationAngle
			)
		}
	...
	drawHUD(context) {
		...
		if (this.playerActor) {
			if (this.playerActor.weapon) {
				if (
					this.playerActor.weapon.crosshairs &&
					this.playerActor.aimLocation
				) {
					context.translate(this.positionOffset.x, this.positionOffset.y)
		...
		if (this.playerActor) {
			...
			var currentObj = this.playerActor.isBuilding
				? this.playerActor.buildingID
				: this.playerActor.weaponSwapID
			for (var i = 0; i < 5; i++) {
				context.fillStyle =
			...
			if (this.playerActor.isBuilding) {
				for (var i = 0; i < this.playerActor.blocks.length; i++) {
					...
					context.fillStyle =
						this.playerActor.score >= this.playerActor.blocks[i].cost
			...
			// Weapon
				...
				for (var i = 0; i < this.playerActor.backpackWeapons.length; i++) {
					...
					var drawWeapon = ConvertToDrawActor(
						this.playerActor.backpackWeapons[i]
					)
	...
	drawTouchControls(touchPos) {
		...
		if (touchPos != null) {
			...
			var convertedPos = new Pair(
				touchPos.clientX - rect.left,
